...
data class UserFailedAttempts(val failedAttempts: Int)
...
fun Handle.upsertAppUserFromAd(adUser: AdUser, user: AuthenticatedUser): AppUser =
    createQuery(
            // language=SQL
            """
...
fun Handle.userIsLockedOrInDelayPeriod(email: String): String? {
    ...
    if (userStatus != null) {
        if (userStatus.isLocked && LocalDateTime.now().isBefore(userStatus.lockoutExpiration)) {
            ...
            userStatus.delayUntil != null && LocalDateTime.now().isBefore(userStatus.delayUntil)
        ) {
...
/**
 * Record a failed login attempt and set progressive delay or lock account if necessary Example
 * workflow logic First 2 failed attempts: No delay. 3rd failed attempt: 5 seconds delay. 4th failed
 * attempt: 10 seconds delay. 5th failed attempt: 20 seconds delay, and a 5 minutes lockout if the
...
fun Handle.loginFailed(email: String) {
    ...
    // - If the number of failed attempts is greater than or equal to 2, the delay increases
    // exponentially
    val delayTime =
        baseDelay * 2.0.pow(maxOf(0, currentFailedAttempts.failedAttempts - 2).toDouble()).toInt()
